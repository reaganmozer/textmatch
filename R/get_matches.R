...
get_matches = function(dist, Z, dist.name, caliper_fun=NULL){
  ...
  if(is.null(caliper_fun)){match = optmatch::fullmatch(dist+caliper(dist,width=calip.val),data=tmp0,tol=0)}
  else{match = optmatch::fullmatch(dist+caliper(dist,width=calip.val)+caliper_fun,data=tmp0,tol=0)}
